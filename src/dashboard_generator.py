...
<body>
    ...
    <!-- Mobile & Tablet Bottom Quick Navigation Dock -->
    <div class="mobile-bottom-nav">
        <button class="mobile-nav-item active" id="mob-nav-leaderboard" onclick="switchMainTab('leaderboard')">
            <span class="mob-nav-icon">🏆</span>
            <span class="mob-nav-label">Leaderboard</span>
        </button>
        <button class="mobile-nav-item" id="mob-nav-fixtures" onclick="scrollToSection('fixture-ticker-section')">
            <span class="mob-nav-icon">⚽</span>
    ...
    <script>
        ...
        function scrollToSection(id) {{
            const el = document.getElementById(id) || document.getElementsByClassName(id)[0];
            if (el) el.scrollIntoView({{ behavior: 'smooth' }});
        }}

        function updateBottomNavActive(tabKey) {{
            document.querySelectorAll('.mobile-nav-item').forEach(el => el.classList.remove('active'));
            if (tabKey === 'leaderboard') {{
                document.getElementById('mob-nav-leaderboard')?.classList.add('active');
            }} else if (tabKey && tabKey.startsWith('gw_')) {{
                document.getElementById('mob-nav-gw')?.classList.add('active');
            }}
        }}
